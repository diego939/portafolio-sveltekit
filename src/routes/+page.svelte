	...
	<section class="text-center py-6 sm:py-8 md:py-12 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl sm:rounded-2xl md:rounded-3xl mx-4 sm:mx-6">
		<div class="max-w-4xl mx-auto px-4 sm:px-6">
			...
			<h1 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-3 sm:mb-4 md:mb-6 px-2">
				Hola, soy <span class="text-blue-600">{nombre}</span>
			</h1>
			...
			<div class="flex flex-wrap justify-center gap-1 sm:gap-1.5 mb-4 sm:mb-6 md:mb-8 px-2">
				{#each tecnologias as tecnologia}
					<span class="px-1.5 py-0.5 sm:px-2 sm:py-0.5 bg-blue-100 text-blue-800 border border-blue-200 rounded-full text-xs font-medium">
						{tecnologia}
					</span>
				{/each}
			</div>
	...
	<section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 px-4 sm:px-6">
		{#each destacados as destacado}
			<div class="bg-white p-4 sm:p-6 md:p-8 rounded-xl shadow-lg hover:shadow-xl transition-shadow flex flex-col items-center text-center h-full">
			   ...
			   class:text-blue-600={destacado.color==='blue'}
			   class:text-green-600={destacado.color==='green'}
			   class:text-purple-600={destacado.color==='purple'}
			   class:text-indigo-600={destacado.color==='indigo'}
			   class:hover:text-blue-700={destacado.color==='blue'}
			   class:hover:text-green-700={destacado.color==='green'}
			   class:hover:text-purple-700={destacado.color==='purple'}
			   class:hover:text-indigo-700={destacado.color==='indigo'}>
				Ver más →
			</a>
			</div>
		{/each}
	...
	<section class="bg-white rounded-xl shadow-lg p-4 sm:p-6 md:p-8 text-center mx-4 sm:mx-6">
		...
		<div class="flex flex-wrap justify-center gap-2 mb-6 sm:mb-8 px-2">
			<a href="https://www.linkedin.com/in/diegodavidalmiron" target="_blank" rel="noopener noreferrer"
				class="flex items-center px-2 sm:px-3 py-1 bg-white border border-blue-600 text-blue-700 rounded-full shadow-sm text-xs font-semibold hover:bg-blue-50 transition-colors">
				<svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1 text-blue-600" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-10h3v10zm-1.5-11.268c-.966 0-1.75-.784-1.75-1.75s.784-1.75 1.75-1.75 1.75.784 1.75 1.75-.784 1.75-1.75 1.75zm15.5 11.268h-3v-5.604c0-1.337-.025-3.063-1.868-3.063-1.868 0-2.154 1.459-2.154 2.967v5.7h-3v-10h2.881v1.367h.041c.401-.761 1.379-1.563 2.841-1.563 3.039 0 3.6 2.001 3.6 4.601v5.595z"/></svg>
				Diego David Almirón
			</a>
			<a href="mailto:diegodavidalmiron17@gmail.com" class="flex items-center px-2 sm:px-3 py-1 bg-white border border-blue-600 text-blue-700 rounded-full shadow-sm text-xs font-semibold hover:bg-blue-50 transition-colors">
				<svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1 text-blue-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
					<path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="m3.5 5.5 7.893 6.036a1 1 0 0 0 1.214 0L20.5 5.5M4 19h16a1 1 0 0 0 1-1V6a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1Z"/>
				  </svg>
				diegodavidalmiron17@gmail.com
			</a>
			<a href="https://wa.me/543795315483" target="_blank" rel="noopener noreferrer" class="flex items-center px-2 sm:px-3 py-1 bg-white border border-blue-600 text-blue-700 rounded-full shadow-sm text-xs font-semibold hover:bg-blue-50 transition-colors">
				<svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1 text-blue-600" fill="currentColor" viewBox="0 0 24 24"><path d="M20.52 3.48A12.07 12.07 0 0 0 12 0C5.37 0 0 5.37 0 12c0 2.12.55 4.19 1.6 6.01L0 24l6.18-1.62A11.94 11.94 0 0 0 12 24c6.63 0 12-5.37 12-12 0-3.21-1.25-6.23-3.48-8.52zM12 22c-1.85 0-3.66-.5-5.23-1.44l-.37-.22-3.67.96.98-3.58-.24-.37A9.94 9.94 0 0 1 2 12c0-5.52 4.48-10 10-10s10 4.48 10 10-4.48 10-10 10zm5.2-7.8c-.28-.14-1.65-.81-1.9-.9-.25-.09-.43-.14-.61.14-.18.28-.7.9-.86 1.08-.16.18-.32.2-.6.07-.28-.14-1.18-.44-2.25-1.41-.83-.74-1.39-1.65-1.55-1.93-.16-.28-.02-.43.12-.57.13-.13.28-.34.42-.51.14-.17.18-.29.28-.48.09-.19.05-.36-.02-.5-.07-.14-.61-1.47-.84-2.01-.22-.53-.45-.46-.62-.47-.16-.01-.36-.01-.56-.01-.19 0-.5.07-.76.34-.26.27-1 1-.98 2.43.02 1.43 1.03 2.81 1.18 3 .15.19 2.03 3.1 4.93 4.23.69.3 1.23.48 1.65.61.69.22 1.32.19 1.81.12.55-.08 1.65-.67 1.89-1.32.23-.65.23-1.2.16-1.32-.07-.12-.25-.19-.53-.33z"/></svg>
				+54 379 531-5483
			</a>
			<a href="#" on:click|preventDefault={() => contactModalOpen.set(true)} class="flex items-center px-2 sm:px-3 py-1 bg-white border border-blue-600 text-blue-700 rounded-full shadow-sm text-xs font-semibold hover:bg-blue-50 transition-colors">
				<svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1 text-blue-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 10.5V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-4.5"/><path d="M16 3v4"/><path d="M8 3v4"/><path d="M3 10h18"/></svg>
